...
1. 다음 설정을 사용하여 새 게스트 사용자를 만듭니다(다른 설정은 기본값으로 유지).
...
1. Contoso 랩 Azure AD 테넌트의 **Contoso 랩- 개요** 블레이드로 이동하여 **테넌트 삭제**를 클릭하고 **'Contoso 랩’ 테넌트 삭제** 블레이드에서 **Azure 리소스 삭제 권한 가져오기** 링크를 클릭한 후 Azure Active Directory의 **속성** 블레이드에서 **Azure 리소스 액세스 관리**를 **예**로 설정하고 **저장**을 클릭합니다.

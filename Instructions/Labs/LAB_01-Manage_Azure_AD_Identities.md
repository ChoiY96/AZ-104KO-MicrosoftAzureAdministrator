...
1. **InPrivate** 브라우저 창을 열고 새로 만든 사용자 계정을 사용하여 [Azure Portal](https://portal.azure.com)에 로그인합니다. 암호를 업데이트하라는 메시지가 표시되면 암호를 **Pa55w.rd1234**로 변경합니다.
...
1. Contoso 랩 Azure AD 테넌트의 **Contoso 랩 - 개요** 블레이드로 이동하여 **테넌트 관리**를 클릭한 후 다음 화면에서 **Contoso 랩** 옆의 상자를 선택하고, **삭제**를 클릭하고, **'Contoso 랩' 테넌트 삭제?** 블레이드에서 **Azure 리소스 삭제 권한 가져오기** 링크를 클릭하고, Azure Active Directory의 **속성** 블레이드에서 **Azure 리소스에 대한 액세스 관리**를 **예**로 설정한 후 **저장**을 클릭합니다.

1. **'Contoso 랩’ 테넌트 삭제** 블레이드로 다시 이동하여 **새로 고침**을 클릭하고 **삭제**를 클릭합니다.

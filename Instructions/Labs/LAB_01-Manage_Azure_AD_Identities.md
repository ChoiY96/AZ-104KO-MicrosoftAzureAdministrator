...
+ 작업 2: 할당된 동적 구성원 자격이 있는 Azure AD 그룹 만들기
...
1. **편집** 을 클릭하고 **설정** 섹션에서 **사용 위치**를 **미국**으로 설정하고 변경 내용을 저장합니다.
...
1. 다음 설정을 사용하여 새 사용자를 만듭니다(다른 설정은 기본값으로 유지).
    ...
    >**참고**: 전체 **사용자 계정 이름**(사용자 이름 및 도메인)을 **클립보드에 복사합니다**. 이 작업의 후반에서 필요합니다.
...
1. **InPrivate** 브라우저 창의 Azure Portal에서 **Azure Active Directory**를 검색하고 선택합니다.
    ...
    >**참고**: 이 사용자 계정은 Azure Active Directory 테넌트에 액세스할 수 있지만 Azure 리소스에는 액세스할 수 없습니다. Azure 역할 기반 액세스 제어를 사용하여 이러한 액세스를 명시적으로 부여해야 하므로 예상된 결과입니다. 

1. **InPrivate** 브라우저 창의 Azure AD 블레이드에서 **관리** 섹션으로 아래로 스크롤하여 **사용자 설정**을 클릭합니다. 구성 옵션을 수정할 수 있는 권한이 없다는 것에 유의하세요.

1. **InPrivate** 브라우저 창의 Azure AD 블레이드에서 **관리** 섹션의 **사용자**를 클릭한 다음, **+ 새 사용자**를 클릭합니다.
...
1. Azure Portal에서 az104-aaduser1 사용자를 로그아웃하고 InPrivate 브라우저 창을 닫습니다.

#### 작업 2: 할당된 동적 구성원 자격이 있는 Azure AD 그룹 만들기

이 작업에서는 할당된 동적 구성원 자격이 있는 Azure Active Directory 그룹을 만듭니다.

1. 사용자 계정으로 로그인한 Azure Portal로 돌아가서 Azure AD 테넌트의 **개요** 블레이드로 다시 이동한 다음, **관리** 섹션에서 **라이선스**를 클릭합니다.
...
1. 규칙을 저장하고 **새 그룹** 블레이드로 돌아와서 **만들기**를 클릭합니다. 
...
1. **그룹 - 모든 그룹** 블레이드로 돌아가서 **IT 클라우드 관리자** 그룹을 나타내는 항목을 클릭하여 **구성원** 블레이드를 표시합니다. **az104-01a-aaduser1**가 그룹 구성원 목록에 나타나는지 확인합니다.

1. **그룹- 모든 그룹** 블레이드로 돌아가서 **IT 시스템 관리자** 그룹을 나타내는 항목을 클릭하여 **구성원** 블레이드를 표시합니다. **az104-01a-aaduser2**가 그룹 구성원 목록에 나타나는지 확인합니다.
...
1. Azure Portal 도구 모음에서 **디렉터리 + 구독** 단추(Cloud Shell 단추 바로 오른쪽)를 사용해 기본 Azure AD 테넌트로 다시 전환하세요.

1. **사용자 - 모든 사용자** 블레이드로 다시 이동한 다음 **+ 새 게스트 사용자**를 클릭하세요.

1. 다음 설정을 사용하여 새 게스트 사용자를 만드세요(다른 설정은 기본값으로 유지).

    | 설정 | 값 |
    | --- | --- |
    | 이름 | **az104-01b-aaduser1** |
    | 이메일 주소 | 현재 테넌트의 사용자 주체 이름과 일치하지 않는 유효한 이메일 주소 |
    | 사용 위치 | **미국** |
    | 직위 | **랩 관리자** |
    | 부서 | **IT** |
...
1. **+ 구성원 자격 추가**를 클릭하고 게스트 사용자 계정을 **IT 랩 관리자** 그룹에 추가하세요.
...
1. **Azure  Active Directory Premium P2 - 라이선스가 부여된 사용자** 블레이드로 이동하여 이 랩에서 라이선스를 할당한 사용자 계정을 선택하고 **라이선스 제거**를 클릭하고 확인하라는 메시지가 표시되면 **확인**을 클릭합니다.
...
1. Contoso 랩 Azure AD 테넌트의 **Contoso 랩- 개요** 블레이드로 이동하여 **테넌트 삭제**를 클릭하고 **'Contoso 랩' 디렉터리 삭제** 블레이드에서 **Azure 리소스 삭제 권한 가져오기** 링크를 클릭한 후 Azure Active Directory의 **속성** 블레이드에서 **Azure 리소스 액세스 관리**를 **예**로 설정하고 **저장**을 클릭하세요.

1. Azure Portal에서 로그아웃하고 다시 로그인합니다. 

1. **'Contoso 랩' 디렉터리 삭제** 블레이드로 다시 이동하여 **삭제**를 클릭합니다.

> **참고**: 테넌트를 삭제하려면 라이선스가 만료되어야 합니다. 추가 비용이 발생하지 않습니다.
...
- 할당된 동적 구성원 자격이 있는 Azure AD 그룹 만들기

...
1. Azure Portal에서 **Recovery Services 자격 증명 모음**을 검색 및 선택하고 **Recovery Services 자격 증명 모음** 블레이드에서 **+ 새로 만들기**를 클릭합니다.
...
1. **az104-10-vm1** Azure 가상 머신으로의 원격 데스크톱 세션 내에서 웹 브라우저를 시작하고 [Azure Portal](https://portal.azure.com)로 이동한 후 자격 증명을 사용하여 로그인합니다. 
...
1. **Az104-10-vm1** 에 대한 원격 데스크톱 세션에서, 파일 탐색기를 열고 **C:\\Windows\\System32\\drivers\\etc\\** 폴더로 이동하여 **호스트** 파일을 삭제합니다.
...
1. **az104-10-vm0** Azure 가상 머신으로의 원격 데스크톱 세션 내에서 웹 브라우저를 시작하고 [Azure Portal](https://portal.azure.com)로 이동한 후 자격 증명을 사용하여 로그인합니다. 
...
1. 파일 탐색기를 시작하고 **다운로드** 폴더로 이동하고 새로 다운로드한 파일을 마우스 오른쪽 단추로 클릭하고 나타나는 메뉴에서 **속성**을 선택하고 **속성** 대화 상자에서 **차단 해제** 체크박스를 선택하고 **확인**을 클릭합니다.
...
1. **이 서버와 연결된 백업 데이터가 1개 있습니다. "확인"을 클릭하면 모든 클라우드 백업 데이터가 영구적으로 삭제된다는 것을 알고 있습니다. 이 작업을 취소할 수 없습니다. 이 구독 관리자에게 이 삭제를 통지하는 경고가 발송될 수 있습니다**. 그리고 **삭제**를 클릭합니다.

...
1. **다음: 관리 >**을 차례로 클릭한 다음 **가상 머신 만들기** 블레이드의 **관리** 탭에서 다음 설정을 지정합니다(다른 설정은 기본값으로 유지).
    ...
    >**참고**: 필요할 경우, 드롭다운 목록에서 기존 스토리지 계정을 선택합니다. 스토리지 계정 이름을 기록해둡니다. 다음 작업에서 사용합니다. 
...
1. **사용자 지정 배포**에서 다음 설정을 지정합니다(다른 설정은 기본값으로 유지).
    ...
    >**참고**: 템플릿을 사용하여 배포하는 고유한 리소스의 속성에 해당하는 매개 변수(가상 머신 및 해당 네트워크 인터페이스 포함)를 수정해야 합니다.

1. **위에 명시된 사용 약관에 동의함** 체크박스를 선택하고 **구매**를 클릭합니다.
...
1. **템플릿 편집** 블레이드의 템플릿의 내용을 표시하는 섹션에서 **50**번째 줄인(`                    "dataDisks": [ ]` line)를 다음 코드로 교체합니다.

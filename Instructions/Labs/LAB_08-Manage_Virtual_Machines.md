...
귀하는 Azure 가상 머신의 배포 및 구성을 위한 다양한 옵션을 식별하는 작업을 수행합니다. 먼저 Azure 가상 머신을 사용할 경우 구현할 수 있는 다양한 컴퓨팅 및 스토리지 복원력과 확장성 옵션을 결정해야 합니다. 다음으로, Azure 가상 머신 확장 집합을 사용할 경우 사용할 수 있는 컴퓨팅 및 저장소 복원력과 확장성 옵션을 조사해야 합니다. 또한 Azure 가상 머신 사용자 지정 스크립트 확장을 사용하여 가상 머신 및 가상 머신 확장 집합을 자동으로 구성하는 기능을 탐색하고자 합니다.

## 목적

이 랩에서는 다음 작업을 수행합니다.

+ 작업 1: Azure Portal과 및 Azure Resource Manager 템플릿을 사용하여 영역 복원력이 있는 Azure 가상 머신을 배포합니다.
+ 작업 2: 가상 머신 확장을 사용하여 Azure 가상 머신을 구성합니다.
+ 작업 3: Azure 가상 머신용 컴퓨팅 및 스토리지 크기를 조정합니다.
+ 작업 4: Microsoft.Insights 및 Microsoft.AlertsManagement 리소스 공급자 등록
+ 작업 5: Azure Portal을 사용하여 영역 복원력이 있는 Azure 가상 머신 확장 집합을 배포합니다.
+ 작업 6: 가상 머신 확장을 사용하여 Azure 가상 머신 확장 집합을 구성합니다.
+ 작업 7: Azure Virtual Machine Scale Sets용 컴퓨팅 및 스토리지 크기 조정 (선택 사항)
...
1. **가상 머신 만들기** 블레이드의 **기본** 탭에서 다음 설정을 지정합니다(나머지는 기본값으로 남겨둡니다).

    | 설정 | 값 | 
    | --- | --- |
    | 구독 | 이 랩에서 사용할 Azure 구독의 이름 |
    | 리소스 그룹 | 새 리소스 그룹 **az104-08-rg01**의 이름 |
    | 가상 시스템 이름 | **az104-08-vm0** |
...
1. **다음: 디스크 >**, **가상 머신 만들기** 블레이드의 **디스크** 탭에서 다음 설정을 지정합니다(나머지는 기본값을 그대로 유지).
...
1. **다음: 네트워킹 >**, **가상 머신 만들기** 블레이드의 **네트워킹** 탭에서 **Virtual network** 텍스트 상자 아래의 **새로 만들기**를 클릭하세요.
...
1. **확인**을 클릭하고, **가상 머신 만들기** 블레이드의 **네트워킹** 탭으로 돌아와서 다음 설정을 지정합니다(나머지는 기본값을 그대로 유지).
...
1. **다음: 관리 >**, **가상 머신 만들기** 블레이드의 **관리** 탭에서 다음 설정을 지정합니다(나머지는 기본값을 그대로 유지).

    | 설정 | 값 | 
    | --- | --- |
    | 부팅 진단 | **사용자 지정 스토리지 계정으로 사용** |
    | 진단 스토리지 계정 | **새로 만들기** |
    | 스토리지 계정 만들기 | 소문자로 전역으로 고유한 이름 입력 > **확인** 클릭 |

    >**참고**: 진단 스토리지 계정의 이름을 식별합니다. 다음 작업에서 사용됩니다. 
    
1. **다음: 고급 >**, **가상 머신 만들기**의 **고급** 탭에서, 아무것도 수정하지 말고 사용 가능한 설정을 검토하고, **검토 + 만들기**를 클릭합니다.
...
1. **사용자 지정 배포** 에서 다음 설정을 지정합니다(나머지는 기본값을 그대로 유지).
    ...
    >**참고**: 가상 머신과 네트워크 인터페이스를 포함, 템플릿을 사용하여 배포하는 고유한 리소스의 속성에 해당하는 매개 변수를 수정해야 합니다. 두 개의 가상 머신으로 구성된 배포를 영역 중복으로 지정하려면 다른 가용성 영역을 지정해야 합니다.

1. **위에 명시된 이용 약관에 동의함** 확인란을 선택하고 **구매**를 클릭합니다.

    >**참고**: 다음 작업을 진행하기 전에 두 배포가 모두 완료될 때까지 기다립니다. 배포가 완료되려면 5분 정도 걸립니다.
...
1. 스토리지 계정 블레이드에서 **컨테이너**를 클릭한 다음 **+ 컨테이너**를 클릭합니다. 

1. **새 컨테이너** 블레이드에서 다음 설정을 지정하고(나머지는 기본값을 그대로 유지) **만들기**를 클릭합니다.
...
1. **Blob 업로드** 블레이드에서 폴더 아이콘을 선택하고, **열기**대화 상자에서 **\\Allfiles\\Labs\\08** 폴더로 이동하여**az104-08-install_IIS.ps1**을 선택하고 **열기**를 선택하고 **Blob 업로드** 블레이드로 돌아와 **업로드**를 클릭합니다. 
...
1. **템플릿 편집** 블레이드에서 템플릿의 내용을 표시하는 섹션에 다음 코드를 줄 **20**부터("리소스`": [` 라인 바로 아래)에 삽입합니다.

 >**참고**: 코드를 한 줄씩 붙여넣는 도구를 사용하는 경우 Intellisense가 유효성 검사 오류를 일으키는 괄호를 추가할 수 있습니다. 코드를 먼저 메모장에 붙여 넣은 다음 20번째 줄에 붙여 넣으세요. 
...
1. **저장**을 클릭하고 **사용자 지정 템플릿** 블레이드에서 **위에 명시된 사용 약관에 동의함** 확인란을 선택하고 **구매**를 클릭합니다.

    >**참고**: **템플릿에 있는 하나 이상의 리소스가 지원하지 않는 위치에 리소스 그룹이 있습니다**는 메시지를 무시합니다.**다른 리소스 그룹을 선택하세요**. 예상된 결과이고, 이 경우 무시할 수 있습니다.
...
1. **실행 명령 스크립트** 블레이드에서 다음을 입력하고 **실행**을 클릭하여 **az104-08-vm0**에서 호스팅되는 웹 사이트에 액세스합니다.
   ...
   ```
    ...
    >**참고**: **az104-08-vm0**에 연결하고 `Invoke-WebRequest -URI http://10.80.0.5` 을 실행하여 **az104-08-vm1**에서 호스팅되는 웹 사이트에 액세스할 수도 있습니다.
...
1. 다음 설정을 사용하여 관리 디스크를 만듭니다(나머지는 기본값을 그대로 유지).
...
1. 다시 **az104-08-vm0 - 디스크** 블레이드에서 **저장**을 클릭합니다.
...
1. **템플릿 편집** 블레이드의 템플릿의 내용을 표시하는 섹션에서 **30**번째 줄인 `"vmSize"을 다음으로 교체합니다. "Standard_D2s_v3"`를 다음으로 교체합니다.
...
1. **템플릿 편집** 블레이드의 템플릿의 내용을 표시하는 섹션에서 **49**번째 줄인 (`"dataDisks": [ ]` line)를 다음 코드로 교체합니다.
...
1. Azure Portal에서 **Virtual Machine Scale Sets**를 검색 및 선택하고 **Virtual Machine Scale Sets** 블레이드에서 **+ 추가**를 클릭합니다.

1. **Virtual Machine Scale Sets 만들기** 블레이드의 **기본** 탭에서 다음 설정을 지정하고(나머지는 기본값을 그대로 유지) **다음: 디스크 >** 를 클릭합니다:
    ...
    | 지역 | 가용성 영역을 지원하고, 이 랩 앞부분에서 가상 머신을 배포하는 데 사용한 것과 다른 Azure 가상 머신을 프로비저닝할 수 있는 영역 중 하나를 선택합니다. | 
    | 가용성 영역 | **영역 1, 2, 3** |
...
1. **가상 머신 확장 집합 만들기** 블레이드의 **디스크** 탭에서 기본 값을 수락하고 **다음 :**을 클릭합니다.** 네트워킹 >**.
...
1. **가상 머신 확장 집합 만들기** 블레이드의 **네트워킹** 탭으로 돌아와서 다음 설정을 지정하고(나머지는 기본값을 그대로 유지) **다음 :**을 클릭합니다.** 크기 조정 >**:
    ...
    | 백 엔드 풀을 선택합니다. | **(신규) bepool** |
    
1. **가상 머신 확장 집합 만들기** 블레이드의 **크기 조정** 탭에서 다음 설정을 지정하고(나머지는 기본값을 그대로 유지) **다음 :**을 클릭합니다.** 관리 >**:
...
1. **가상 머신 확장 집합 만들기** 블레이드의 **관리** 탭에서 **부트 진단** 옵션이 활성화되어 있는지 확인하고, **스토리지 계정 만들기** 블레이드에서 **새로 만들기**를 선택한 후 **이름** 텍스트 상자에 고유하고 유효한 스토리지 계정 이름을 입력하고 **확인**을 클릭한 후 **다음 :**을 클릭합니다.** 상태 >**:

    >**참고**: 다음 작업에서 이 스토리지 계정의 이름이 필요합니다.

1. **가상 머신 확장 집합 만들기** 블레이드의 **상태** 탭에서, 기본 설정을 변경하지 않고 검토한 후 **다음: **을 클릭합니다.** 고급 >**.

1. **가상 머신 확장 집합 만들기** 블레이드의 **고급** 탭에서 다음 설정을 지정하고(나머지는 기본값을 유지) **검토 + 만들기**를 클릭합니다.
...
1. Azure Portal에서 **Virtual Machine Scale Sets** 블레이드로 돌아간 후 **az10408vmss0**를 클릭합니다.

1. **az10408vmss0** 블레이드에서 **확장**을 클릭하고 **+ 추가**를 클릭합니다.

1. **새 리소스** 블레이드에서 **사용자 지정 스크립트 확장**을 클릭한 다음 **만들기**를 클릭합니다.

1. **확장 설치**블레이드에서 이 작업 초기에 스토리지 계정의 **스크립트** 컨테이너에 업로드된 **az104-08-install_IIS.ps1** 스크립트를 **찾아보기** 후 **선택**하고 **확인**을 클릭합니다.

    >**참고**: 확장 설치가 완료될 때까지 기다린 후 다음 작업을 진행합니다.

1. **az10408vmss0** 블레이드의 **설정** 섹션에서 **인스턴스**를 클릭하고, 가상 머신 확장 집합의 두 인스턴스 옆에 있는 확인란을 선택하고 **업그레이드**를 클릭한 다음, 확인을 요청하면 **예**를 클릭합니다.

    >**참고**: 업그레이드가 완료될 때까지 기다린 후 다음 작업을 진행합니다.

1. Azure Portal에서 **부하 분산 장치**를 검색 및 선택하고 부하 분산 장치 목록에서 **az10408vmss0-lb**를 클릭합니다.

1. **az10408vmss0-lb** 블레이드에서 부하 분산 장치의 프런트 엔드에 할당된 **공용 IP 주소**의 값을 기록하고 새 브라우저 탭을 열어 해당 IP 주소로 이동합니다.

    >**참고**: 브라우저 페이지에 Azure 가상 머신 확장 집합 **az10408vmss0**의 인스턴스 중 하나의 이름이 표시되는지 확인합니다.


#### 작업 7: Azure 가상 머신 확장 집합에 사용할 컴퓨팅 및 스토리지 크기를 조정

이 작업에서는 가상 머신 확장 집합 인스턴스의 크기를 변경하고 자동 크기 조정 설정을 구성하고 디스크를 연결합니다.

1. Azure Portal에서 **가상 머신 확장 집합**을 선택하고 az10408vmss0 확장 집합을 선택합니다.

1. **az10408vmss0** 블레이드에서 **크기**를 클릭합니다.

1. 사용 가능한 크기 목록에서 **표준 DS1_v2**을 선택하고 **크기 조정**을 클릭합니다.

1. **설정** 섹션에서 **인스턴스**를 클릭하고 가상 머신 확장 집합의 두 인스턴스 옆에 있는 확인란을 선택한 후 **업그레이드**를 클릭한 다음 확인을 요청하면 **예**를 클릭합니다.
...
1. **az10408vmss0 - 인스턴스** 블레이드로 돌아가서 **크기 조정**을 클릭합니다.

1. **az10408vmss0 - 크기 조정** 블레이드에서, **사용자 지정 자동 크기 조정** 옵션을 선택하고 다음 설정으로 자동 크기 조정을 구성합니다(나머지는 기본값을 그대로 유지).

    | 설정 | 값 |
    | --- |--- |
    | 배율 모드 | **메트릭을 기반으로 배율 조정** | 

1. **+ 규칙 추가** 링크를 클릭하고 **크기 조정 규칙** 블레이드에서 다음 설정을 지정합니다(나머지는 기본값을 그대로 남겨둡니다).
...
1. **추가**를 클릭하고, 다시 **az10408vmss0 - 크기 조정** 블레이드에서 다음 설정을 지정합니다 (나머지는 기본값을 그대로 유지):
...
1. 클라우드 쉘 창에서 다음을 실행하여 Azure 가상 머신 확장 집합 **az10408vmss0** 앞에 있는 부하 분산 장치의 공용 IP 주소를 식별합니다.
   ...
   ```
1. Cloud Shell 창에서 다음을 실행하여 Azure 가상 머신 확장 집합 **az10408vmss0** 인스턴스에서 호스팅되는 웹 사이트에 HTTP 요청을 전송하는 시작 및 무한 루프를 실행합니다.
...
1. 세 번째 인스턴스가 프로비전되면 블레이드로 이동하여 **위치**를 결정합니다 (이 작업 이전에 식별한 처음 두 영역과 달라야합니다. 

1. Cloud Shell 창을 닫습니다. 

1. **az10408vmss0** 블레이드에서 **디스크**를 클릭하고, **+ 데이터 디스크 추가**를 클릭한 후, 다음 설정으로 새 관리 디스크를 연결합니다 (나머지는 기본값을 그대로 유지):
...
1. 변경 내용을 저장하고, **az10408vmss0** 블레이드의 **설정** 섹션에서 **인스턴스**를 클릭하고 가상 머신 확장 집합의 두 인스턴스 옆에 확인란을 선택한 후 **업그레이드**를 클릭한 다음, 확인 메시지가 표시 되면 **예**를 클릭합니다.
...
1. 다음 명령을 실행하여 az104-08-configure_VMSS_disks.ps1를 제거합니다.
...
- Azure Portal 및 Azure Resource Manager 템플릿을 사용하여 영역 복원력이 있는 Azure 가상 머신을 배포하였습니다.
- 가상 머신 확장을 사용하여 Azure 가상 머신을 구성하였습니다.
- Azure 가상 머신용 컴퓨팅 및 스토리지 크기를 조정하였습니다.
- Azure Portal을 사용하여 영역 복원력이 있는 Azure 가상 머신 확장 집합을 배포하였습니다.
- 가상 머신 확장을 사용하여 Azure 가상 머신 확장 을 구성하였습니다.
- Azure 가상 머신 확장 집합에 사용할 컴퓨팅 및 스토리지 크기를 조정하였습니다.

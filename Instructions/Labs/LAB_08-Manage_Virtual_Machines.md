...
1. **다음: 네트워킹 >** 을 차례로 클릭하고 **가상 머신 만들기** 블레이드의 **네트워킹** 탭에서 **가상 네트워크** 텍스트 상자 아래의 **새로 만들기**를 클릭하세요.

1. **가상 네트워크 만들기** 블레이드에서 다음 설정을 지정합니다(다른 설정은 기본값으로 유지).
...
1. **다음: 관리 >** 을 차례로 클릭한 다음 **가상 머신 만들기** 블레이드의 **관리** 탭에서 다음 설정을 지정합니다(다른 설정은 기본값으로 유지).

    | 설정 | 값 | 
    | --- | --- |
    | 부팅 진단 | **사용자 지정 스토리지 계정으로 사용** |
    | 진단 스토리지 계정 | **새로 만들기** |
    | 스토리지 계정 만들기 | 전역 고유 이름을 모두 소문자로 입력하고 **확인** 클릭|

    >**참고**: 진단 스토리지 계정의 이름을 식별합니다. 다음 작업에서 사용합니다. 
    
1. **다음: 고급 >** 을 차례로 클릭하고 **가상 머신 만들기** 블레이드의 **고급** 탭에서, 아무것도 수정하지 말고 사용 가능한 설정을 검토하고, **검토 + 만들기**를 클릭합니다.
...
1. **사용자 지정 배포**에서 다음 설정을 지정합니다(다른 설정은 기본값으로 유지).
    ...
    >**참고**: 템플릿을 사용하여 배포하는 고유한 리소스의 속성에 해당하는 매개 변수(가상 머신 및 해당 네트워크 인터페이스 포함)를 수정해야 합니다. 두 개의 가상 머신으로 구성된 배포를 영역 중복으로 지정하려면 다른 가용성 영역을 지정해야 합니다.

1. **위에 명시된 이용 약관에 동의함** 확인란을 선택하고 **구매**를 클릭합니다.

    >**참고**: 다음 작업으로 진행하기 전에 두 배포가 모두 완료될 때까지 기다립니다. 배포가 완료되려면 약 5분 소요됩니다.
...
1. **템플릿 편집** 블레이드에서 템플릿의 내용을 표시하는 섹션에 다음 코드를 **20**번째 줄(`    "resources": [` 줄 바로 아래)에 삽입합니다.

 >**참고**: 코드를 한 줄씩 붙여넣는 도구를 사용하는 경우 IntelliSense가 유효성 검사 오류를 일으키는 괄호를 추가할 수 있습니다. 코드를 먼저 메모장에 붙여넣은 다음 20번째 줄에 붙여넣습니다. 
...
1. **저장**을 클릭하고 **사용자 지정 템플릿** 블레이드에서 **위에 명시된 사용 약관에 동의함** 확인란을 선택하고 **구매**를 클릭합니다.
    ...
    >**참고**: **템플릿에 있는 하나 이상의 리소스가 지원하지 않는 위치에 리소스 그룹이 있습니다**라는 메시지를 무시합니다.**다른 리소스 그룹을 선택하세요**. 예상된 결과이고, 이 경우 무시하세요.

    >**참고**: 템플릿 배포가 완료될 때까지 기다립니다. **az104-08-vm0** 및 **az104-08-vm1** 가상 머신의 **확장** 블레이드에서 진행 상황을 모니터링할 수 있습니다. 작업은 3분 이내에 완료됩니다.

1. 사용자 지정 스크립트 확장 기반 구성을 성공했는지 확인하려면 **az104-08-vm1** 블레이드로 다시 이동한 후 **작업** 섹션에서 **실행 명령**을 클릭하고 명령 목록에서 **RunPowerShellScript**를 클릭합니다.

1. **실행 명령 스크립트** 블레이드에서 다음 명령을 입력하고 **실행**을 클릭하여 **az104-08-vm0**에서 호스팅되는 웹 사이트에 액세스합니다.
   ...
   ```
    ...
    >**참고**: **az104-08-vm0**에 연결하고 `Invoke-WebRequest -URI http://10.80.0.5`를 실행하여 **az104-08-vm1**에서 호스팅되는 웹 사이트에 액세스할 수도 있습니다.
...
1. 다시 **az104-08-vm0 - 디스크** 블레이드로 돌아가서 **저장**을 클릭합니다.

1. **az104-08-vm0** 블레이드의 **작업** 섹션에서 **실행 명령**을 클릭하고 명령 목록에서 **RunPowerShellScript**를 클릭합니다.

1. **실행 명령 스크립트** 블레이드에서 다음을 입력하고 **실행**을 클릭하여 간단한 레이아웃과 고정 프로비전을 사용하여 새로 연결된 두 개의 디스크로 구성된 드라이브 Z:를 만듭니다:
...
1. **템플릿 편집** 블레이드의 템플릿의 내용을 표시하는 섹션에서 **30**번째 줄인 `                    "vmSize": "Standard_D2s_v3"`를 다음으로 교체합니다.
   ...
   ```
    ...
    >**참고**: 이 템플릿의 해당 섹션에서는 Azure Portal을 통해 첫 번째 가상 머신에 대해 지정한 것과 동일한 Azure 가상 머신 크기를 정의합니다.
...
1. Azure Portal에서 **가상 머신 확장 집합**을 검색 및 선택하고 **가상 머신 확장 집합** 블레이드에서 **+ 추가**를 클릭합니다.
...
1. **가상 머신 확장 집합 만들기** 블레이드의 **기본** 탭에서 다음 설정을 지정하고(다른 설정은 기본값으로 유지) **다음: 디스크 >** 를 클릭합니다.
    ...
    | 지역 | 가용성 영역을 지원하고, 이 랩 전반부에서 가상 머신을 배포하는 데 사용한 것과 다른 Azure 가상 머신을 프로비전할 수 있는 영역 중 하나를 선택합니다. | 
    | 가용성 영역 | **영역 1, 2, 3** |
    ...
    >**참고**: Windows 가상 머신을 가용성 영역에 배포하는 것을 지원하는 Azure 지역 목록은 [Azure의 가용성 영역이란 무엇입니까?](https://docs.microsoft.com/ko-kr/azure/availability-zones/az-overview)를 참조하세요.

1. **가상 머신 확장 집합 만들기** 블레이드의 **디스크** 탭에서 기본 값을 수락하고 **다음 : 네트워킹 >** 을 차례로 클릭합니다.

1. **가상 머신 확장 집합 만들기** 블레이드의 **네트워킹** 탭에서 **가상 네트워크** 텍스트 상자 아래 **가상 네트워크 만들기** 링크를 클릭하고 다음 설정을 사용하여 새 가상 네트워크를 만듭니다(다른 설정은 기본값으로 유지).
...
1. **인바운드 규칙 추가**를 클릭하고 다음 설정을 사용하여 인바운드 보안 규칙을 추가합니다(다른 설정은 기본값으로 유지).
...
1. **공용 IP 주소** 섹션의 **네트워크 인터페이스 편집** 블레이드로 돌아와서 **사용**을 클릭하고 **확인** 을 클릭합니다.

1. **가상 머신 확장 집합 만들기** 블레이드의 **네트워킹** 탭으로 돌아와서 다음 설정을 지정합니다(다른 설정은 기본값으로 유지). 그리고 **다음: 크기 조정 >** 을 차례로 클릭합니다.
...
1. **가상 머신 확장 집합 만들기** 블레이드의 **관리** 탭에서 **부트 진단** 옵션이 사용 설정되어 있는지 확인하고, **새로 만들기**를 선택합니다. **스토리지 계정 만들기** 블레이드에서 **이름** 텍스트 상자에 고유하고 유효한 스토리지 계정 이름을 입력하고 **확인**을 클릭한 후 **다음: 상태 >** 를 차례로 클릭합니다.

    >**참고**: 다음 작업에서 이 스토리지 계정의 이름이 필요합니다.

1. **가상 머신 확장 집합 만들기** 블레이드의 **상태** 탭에서, 기본 설정을 변경하지 않고 검토한 후 **다음:  고급 >*을 차례로 클릭합니다
...
1. **확장 설치** 블레이드에서 이 작업 초기에 스토리지 계정의 **스크립트** 컨테이너에 업로드된 **az104-08-install_IIS.ps1** 스크립트를 **찾아보기**한 후 **선택**하고 **확인**을 클릭합니다.
...
1. **az10408vmss0 - 인스턴스** 블레이드로 돌아가서 **스케일링**을 클릭합니다.
...
1. Cloud Shell 창에서 다음 명령을 실행하여 Azure 가상 머신 확장 집합 **az10408vmss0** 앞에 있는 부하 분산 장치의 공용 IP 주소를 식별합니다.
   ...
   ```
1. Cloud Shell 창에서 다음 명령을 실행하여 Azure 가상 머신 확장 집합 **az10408vmss0** 인스턴스에서 호스팅되는 웹 사이트에 HTTP 요청을 전송하는 시작 및 무한 루프를 실행합니다.
...
1. **az10408vmss0** 블레이드에서 **디스크**를 클릭하고, **+ 데이터 디스크 추가**를 클릭한 후, 다음 설정을 사용하여 새 관리 디스크를 연결합니다(다른 설정은 기본값으로 유지).
...
1. 변경 사항을 저장하고, **az10408vmss0** 블레이드의 **설정** 섹션에서 **인스턴스**를 클릭하고 가상 머신 확장 집합의 두 인스턴스 옆에 확인란을 선택한 후 **업그레이드**를 클릭한 다음, 확인 메시지가 표시 되면 **예**를 클릭합니다.
...
1. Cloud Shell 창에서 다음 명령을 실행하여 스크립트의 내용을 표시합니다.
...
1. 다음 명령을 실행하여 이 모듈의 전체 랩에서 만든 모든 리소스 그룹을 삭제합니다.
   ...
   ```
    ...
    >**참고**: 명령은 비동기적으로 실행되므로(-AsJob 매개 변수에 의해 결정됨), 동일한 PowerShell 세션 내에서 즉시 다른 PowerShell 명령을 실행할 수 있지만 리소스 그룹이 실제로 제거되기까지 몇 분 정도 소요됩니다.

...
1. **위에 명시된 이용 약관에 동의함** 체크박스를 선택하고 **구매**를 클릭합니다.
...
1. 스토리지 계정 블레이드의 **Blob 서비스** 섹션에서 **컨테이너**와 **+ 컨테이너**를 차례로 클릭합니다.
...
1. **저장**을 클릭하고 **사용자 지정 템플릿** 블레이드에서 **위에 명시된 사용 약관에 동의함** 체크박스를 선택하고 **구매**를 클릭합니다.
...
1. Azure Portal에서 **가상 머신 확장 집합**을 검색 및 선택하고 **가상 머신 확장 집합** 블레이드에서 **+ 추가**를 클릭한 다음 **+ 가상 머신**을 클릭합니다.
...
1. **가상 머신 확장 집합 만들기** 블레이드의 **기본** 탭에서 다음 설정을 지정하고(다른 설정은 기본값으로 유지) **다음: 디스크 >** 를 클릭합니다.
    ...
    >**참고**: Windows 가상 머신을 가용성 영역에 배포하는 것을 지원하는 Azure 지역 목록은 [Azure의 가용성 영역이란 무엇입니까?](https://docs.microsoft.com/en-us/azure/availability-zones/az-overview)를 참조하세요.
...
1. Cloud Shell 창에서 다음을 실행하여 Azure 가상 머신 확장 집합 **az10408vmss0** 인스턴스에서 호스트되는 웹 사이트에 HTTP 요청을 전송하는 시작 및 무한 루프를 실행합니다.
...
1. 변경 사항을 저장하고, **az10408vmss0** 블레이드의 **설정** 섹션에서 **인스턴스**를 클릭하고 가상 머신 확장 집합의 두 인스턴스 옆에 있는 체크박스를 선택하고 **업그레이드**를 클릭한 다음, 확인을 요청하면 **예**를 클릭합니다.

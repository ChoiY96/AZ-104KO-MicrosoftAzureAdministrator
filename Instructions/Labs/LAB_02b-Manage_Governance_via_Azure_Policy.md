...
- 적절하게 태그 지정된 인프라 리소스만 인프라 리소스 그룹에 추가할 수 있도록 합니다
...
1. **다음**을 클릭하여 **수정** 탭을 검토합니다. **관리 ID 만들기** 확인란은 선택하지 않은 상태로 유지합니다. 
...
1. 유효성을 검사를 실패했습니다. **유효성을 검사 실패** 링크를 클릭합니다.** **세부 정보를 보려면 여기를 클릭**하여 **오류** 블레이드를 표시하고 실패의 원인을 식별하세요. 

    >**참고**: 오류 메시지는 리소스 배포가 정책상 허용하지 않음을 명시합니다. 
...
1. 다음 설정을 지정(다른 설정은 기본값으로 유지)하여 할당의 나머지 **기본** 속성을 구성합니다.
    ...
    | 설정 | 값 |
    | --- | --- |
    | 할당 이름 | **누락된 경우 Cloud Shell 리소스 그룹에서 역할 태그 및 인프라 값 상속**|
    | 설명 | **누락된 경우 클라우드 웰 리소스 그룹에서 역할 태그 및 인프라 값 상속**|
    | 정책 적용 | 사용 설정됨 |

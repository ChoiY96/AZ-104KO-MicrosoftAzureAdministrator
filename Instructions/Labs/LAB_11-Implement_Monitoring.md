...
+ 작업 1: 랩 환경 프로비전
+ 작업 2: Azure Log Analytics 작업 영역 및 Azure 자동화 기반 솔루션을 만들고 구성합니다.
...
1. Cloud Shell 창에서 다음을 실행하여 가상 머신을 호스팅할 리소스 그룹을 만듭니다(`[Azure_region]` 자리 표시자를 Azure 가상 머신을 배포하려는 Azure 지역의 이름으로 대체합니다).
    ...
    >**참고**: **작업 영역 매핑 문서**에 참조된 [Log Analytics 작업 영역 지역](https://docs.microsoft.com/ko-kr/azure/automation/how-to/region-mappings)으로 나열된 지역 중 하나를 선택해야 합니다
...
1. Cloud Shell 창에서 다음을 실행하여 첫 번째 가상 네트워크를 만들고 업로드한 템플릿 및 매개 변수 파일을 사용하여 가상 머신을 배포합니다.
   ...
   ```
    ...
    >**참고**: 배포가 완료될 때까지 기다리지 말고 다음 작업을 진행하세요. 배포에는 약 3분이 소요됩니다.
...
#### 작업 3: Azure Log Analytics 작업 영역 및 Azure 자동화 기반 솔루션을 만들고 구성합니다.
...
1. **Log Analytics 작업 영역 만들기** 블레이드의 **기본** 탭에서 다음 설정 **검토 + 만들기**를 클릭한 다음 **만들기**를 클릭합니다.
...
1. **인벤토리** 창의 **Log Analytics 작업 영역** 드롭다운 리스트에서 이 작업 앞부분에서 만든 Log Analytics 작업 영역을 선택하고 **사용**을 클릭합니다.
    ...
    >**참고**: 해당 Log Analytics 솔루션의 설치가 완료될 때까지 기다립니다. 배포가 완료되려면 3분 정도 걸립니다.

    >**참고**: 이렇게 하면 **변경 내용 추적** 솔루션도 자동으로 설치됩니다.

1. 자동화 계정 블레이드의 **업데이트 관리** 섹션에서 **업데이트 관리**를 클릭하고 **사용**을 클릭합니다.

    >**참고**: 설치가 완료될 때까지 기다립니다. 배포가 완료되려면 5분 정도 걸립니다.
...
1. **az104-11-vm0 | 메트릭** 블레이드에서 기본 차트에서 유일하게 사용 가능한 **메트릭 네임스페이스**는 **가상 머신 호스트**뿐이라는 점에 유의합니다.
    ...
    >**참고**: 게스트 수준 진단 설정이 아직 구성되지 않았기 때문에 이와 같은 결과는 예상되었습니다.
...
1. **az104-11-vm0 | 진단 설정** 탭에서 개요 탭에서 **게스트 수준 모니터링 사용**을 클릭합니다.

    >**참고**: 작업이 적용될 때까지 기다립니다. 배포가 완료되려면 3분 정도 걸립니다.

1. **az104-11-vm0 | 진단 설정** 성능 카운터 탭으로 전환하고 사용 가능한 카운터를 검토합니다.

    >**참고**: 기본값으로 CPU, 메모리, 디스크 및 네트워크 카운터가 사용됩니다. 자세한 목록을 보려면 **사용자 지정** 보기로 전환할 수 있습니다.

1. **az104-11-vm0 | 진단 설정** 의 **로그** 블레이드에서 사용 가능한 이벤트 로그 컬렉션 옵션을 검토합니다.
...
1. **az104-11-vm0 - 로그** 블레이드에서 이 랩의 앞부분에서 만든 Log Analytics 작업 영역이 **Log Analytics 작업 영역 선택** 드롭다운 목록에서 선택되었는지 확인하고 **사용**을 클릭합니다.
    ...
    >**참고**: 작업이 완료될 때까지 기다리지 않고 다음 단계를 진행합니다. 작업에는 약 5분이 소요될 수 있습니다.
...
1. **az104-11-vm0 | 메트릭** 블레이드의 기본 차트에는 이 시점에서 **메트릭 네임스페이스** 드롭다운 목록에 **가상 머신 호스트** 항목과 함께 **게스트(클래식)** 항목 또한 포함한다는 것에 유의합니다.
    ...
    >**참고**: 게스트 수준 진단 설정을 사용했기 때문에 이와 같이 예상됩니다. 그러나 **메트릭 네임스페이스** 드롭다운 목록에서 직접 게스트 메모리 메트릭을 사용할 수 있는 옵션이 있습니다. 이 연습의 후반에서 사용할 것입니다.
...
1. **메트릭** 드롭다운 목록에서 **메모리\사용 가능한 바이트 수**를 선택하고 **집계** 드롭다운 목록에서 **최대**를 선택하고 결과 차트를 검토합니다. 
   
#### 작업 6: Azure Monitor 기능 검토

1. Azure Portal에서 **모니터**를 검색하여 선택하고 **모니터 | 개요** 블레이드에서 **메트릭**을 클릭합니다.

1. **범위 선택** 블레이드의 **찾아보기** 탭에서 **az104-11-rg0** 리소스 그룹으로 이동하여 확장하고 리소스 그룹내의 **az104-11-vm0** 가상 머신 옆에 위치한 체크박스를 선택하고 **적용**을 클릭합니다.

    >**참고**: 이렇게 하면 **az104-11-vm0 - 메트릭** 블레이드에서 사용할 수 있는 것과 동일한 뷰 및 옵션을 제공합니다.

1. **메트릭** 드롭다운 목록에서 **백분율 CPU**를 선택하고 **집계** 드롭다운 목록에서 **평균**을 선택하고 결과 차트를 검토합니다.

1. **모니터 | 메트릭** 블레이드의 az104-11-vm0의 평균 백분율 CPU 창에서 **새로운 경고 규칙**을 클릭합니다.
...
1. **경고 규칙 만들기** 블레이드의 **작업 그룹** 섹션에서 **작업 그룹 선택**을 클릭한 다음 **+ 작업 그룹 만들기** 단추를 클릭합니다.

1. **작업 그룹 만들기** 블레이드의 **기본** 탭에서 다음 설정을 지정합니다(나머지 설정은 기본값으로 남겨둡니다).
...
1. **알림 유형** 드롭다운 목록에 포함된 **작업 그룹 만들기** 블레이드의 **알림** 탭에서 **이메일/SMS/푸시/음성**을 선택합니다.

1. **이메일/SMS/푸시/음성** 블레이드에서 **이메일** 확인란을 선택하고 **이메일** 텍스트 상자에 이메일 주소를 입력한 후 나머지는 기본값으로 둔 채로 **확인**을 클릭합니다. **작업 그룹 만들기** 블레이드의 **알림** 탭으로 돌아와 **이름** 텍스트 상자에 **관리자 이메일**을 입력하고 **다음: 작업 >**을 선택합니다.

1. **작업 그룹 만들기** 블레이드에 위치한 **작업** 탭의 **작업 유형** 드롭다운 목록에서 사용 가능한 항목을 확인한 후 아무 것도 변경하지 않은 채 **검토 + 만들기**를 선택합니다.

1. **작업 그룹 만들기** 블레이드의 **검토 + 만들기** 탭에서 **만들기**를 선택합니다.

1. **경고 규칙 세부 정보** 섹션의 **경고 규칙 만들기** 블레이드로 돌아가서 다음 설정을 지정합니다(나머지 설정은 기본값으로 남겨둡니다).
...
1. 메시지가 표시되면 **Student** 사용자 이름과 **Pa55w.rd1234** 암호를 사용하여 로그인합니다.

1. 원격 데스크톱 세션 내에서 **시작**을 클릭하고 **Windows System** 폴더를 확장하고 **명령 프롬프트**를 클릭합니다.

1. 명령 프롬프트에서 다음을 실행하여 **az104-11-vm0** Azure VM의 증가한 CPU 사용률을 트리거합니다.
...
1. Azure Portal에서 **Monitor** 블레이드로 돌아가서 **로그**를 클릭합니다. 

    >**참고**: Log Analytics에 처음으로 액세스한다면 **시작**을 클릭해야 합니다.

1. 필요할 경우 **범위 선택**을 클릭하고 **범위 선택** 블레이드에서 **최근** 탭을 선택한 후 **a104-11-vm0**를 선택합니다. 그리고 **적용**을 클릭합니다.

1. 쿼리 창에 다음 쿼리를 붙여넣고 **실행**을 클릭한 후 결과 차트를 검토합니다.
...
1. 도구 모음에서 **쿼리**를 클릭하고 **쿼리** 창에서 **VM 가용성 추적** 타일을 검색한 후 타일의 **실행** 명령 단추를 클릭합니다. 그리고 결과를 검토합니다.

1. **새 쿼리 1** 탭에서 **테이블** 헤더를 선택한 후 **가상 머신** 섹션의 테이블 목록을 검토합니다.

    >**참고**: 여러 테이블의 이름은 이 랩의 앞부분에서 설치한 솔루션과 일치합니다.

1. **VMComputer** 항목 위를 마우스로 가리킨 다음 **데이터 미리보기** 아이콘을 클릭합니다.  

1. 데이터가 사용 가능한 경우 **업데이트** 창에서 **쿼리 편집기에서 보기**를 클릭합니다.

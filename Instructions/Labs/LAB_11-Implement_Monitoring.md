...
1. **az104-11-vm0 | 메트릭** 블레이드의 기본 차트에는 이 시점에서 **메트릭 네임스페이스** 드롭다운 목록에 **가상 머신 호스트** 항목과 함께 **게스트(클래식)** 항목 또한 포함한다는 것에 유의합니다.
    ...
    >**참고**: 게스트 수준 진단 설정을 사용했기 때문에 이와 같이 예상됩니다.

1. **메트릭** 드롭다운 목록에서 사용 가능한 메트릭 목록을 검토합니다.

    >**참고**: 호스트 수준 모니터링에서만 사용할 경우 목록에는 사용할 수 없는 추가 게스트 수준 메트릭이 포함됩니다: 

1. **메트릭** 드롭다운 목록에서 **메모리\사용 가능한 바이트 수**를 선택하고 **집계** 드롭다운 목록에서 **평균**을 선택하고 결과 차트를 검토합니다. 

#### 작업 6: Azure Monitor 기능 검토

1. Azure Portal에서 **모니터**를 검색하여 선택하고 **모니터 | 개요** 블레이드에서 **메트릭**을 클릭합니다.

1. **범위 선택** 블레이드의 **찾아보기** 탭에서 **az104-11-rg0** 리소스 그룹으로 이동하여 확장하고 리소스 그룹내의 **az104-11-vm0** 가상 머신을 선택하고 **적용**을 클릭합니다.
...
1. **경고 규칙 만들기** 블레이드의 **조건** 에서 기존 조건을 클릭합니다. 
...
1. **규칙 만들기** 블레이드의 **작업 그룹** 섹션에서 **작업 그룹 선택**을 클릭한 다음 **+ 작업 그룹 만들기** 단추를 클릭합니다.

1. **작업 그룹 추가** 블레이드에서 다음 설정을 지정합니다(나머지 설정은 기본값으로 남겨둡니다).
...
1. **작업 그룹 추가** 블레이드의 **작업** 섹션에서 다음 설정을 지정합니다(나머지 설정은 기본값으로 남겨둡니다).

    | 설정 | 값 |
    | --- | --- |
    | 작업 그룹 이름 | **az104-11-ag1 이메일** |
    | 작업 유형 | **이메일/SMS/푸시/음성** |

1. **az104-11-ag1 이메일** 매크로 함수 행에서 **세부 정보 편집**을 클릭합니다.

1. **메일/SMS/푸시/음성** 블레이드에서 **이메일** 확인란을 선택하고 **이메일** 텍스트 상자에 사용자의 이메일 주소를 입력하고 나머지는 기본값으로 남겨두고 **확인**을 클릭하고 **작업 그룹 추가** 블레이드로 돌아가서 **확인**을 다시 클릭합니다.

1. **규칙 만들기** 블레이드로 돌아가서 다음 설정을 지정합니다(나머지 설정은 기본값으로 남겨둡니다).
...
1. **경고 규칙** 만들기를 클릭하고 **규칙 만들기** 블레이드를 닫습니다.
...
1. 명령 프롬프트에서 다음을 실행하여 **호스트** 파일 복원을 원래 위치로 복사합니다.
...
1. 쿼리 창에 다음 쿼리를 붙여넣고 **실행**을 클릭합니다.
...
1. 도구 모음에서 **예제 쿼리**를 클릭하고 **샘플 쿼리로 시작** 창에서 각 탭을 검토하고 **VM 가용성 추적**을 찾아 **실행**을 클릭합니다.

1. 결과 차트를 검토하고 다음 텍스트가 포함된 줄을 제거합니다:

   ```
   | where TimeGenerated > ago(1h)
   ```

    >**참고**: 그 결과, 도구 모음의 **시간 범위** 항목이 **쿼리 설정**에서 **지난 24시간**으로 변경되었습니다. 
...
1. 도구 모음에서 **예제 쿼리**를 클릭하고 **예제 쿼리로 시작** 창에서 각 탭을 검토한 다음 **가상 머신 사용 가능한 디스크 공간**을 찾아 **실행**을 클릭합니다.

...
1. **다음: 모니터링 >** 을 차례로 클릭하고 **웹앱** 블레이드의 **모니터링** 탭에서 **Application Insights 사용** 스위치를 **아니요**로 설정하고 **검토 + 만들기**를 클릭한 다음 **만들기**를 클릭합니다. 

    >**참고**: 일반적으로 **Application Insights**를 사용하지만 이 랩에서는 해당 기능이 사용되지 않습니다.
...
1. **+ 슬롯 추가**를 클릭하고 다음 설정을 사용하여 새 슬롯을 추가합니다. 
...
1. 스테이징 슬롯 블레이드를 검토하고 URL이 프로덕션 슬롯에 할당된 URL과 다르다는 것을 확인합니다.
...
1. 필요한 정보를 채운 다음 **자격 증명 저장**을 클릭합니다. 
...
1. Cloud Shell 창에서 원격 git를 추가하려면 다음을 명령을 실행합니다(`[deployment_user_name]` 와 `[git_clone_url]` 자리 표시자를 이전 작업에서 식별한 **배포 자격 증명** 사용자 이름과 **Git Clone Url**의 값으로 바꿔야 합니다).
...
1. Cloud Shell 창에서 샘플 웹 앱 코드를 로컬 리포지토리에서 Azure 웹앱 스테이징 배포 슬롯으로 푸시하려면 다음 명령을 실행합니다(`[deployment_user_name]` 자리 표시자를 이전 작업에서 식별한 **배포 자격 증명** 사용자 이름의 값으로 바꿔야 합니다).
   ```
...
이 작업에서는 스테이징 슬롯을 프로덕션 슬롯과 교환합니다
...
>**참고**: Microsoft.Insights가 이 랩의 구독에 등록되어 있는지 확인해야 합니다. 이 작업에 대한 자세한 내용은 https://docs.microsoft.com/ko-kr/azure/azure-resource-manager/management/resource-providers-and-types#azure-portal을 참조하세요.
    

이 작업에서는 Azure 웹앱의 자동 크기 조정을 구성하고 테스트합니다. 

1. 웹앱의 프로덕션 슬롯을 표시하는 블레이드의 **설정** 섹션에서 **스케일 아웃(App Service 계획)** 을 클릭합니다.

1. **사용자 지정 자동 크기 조정**을 클릭합니다. 

    >**참고**: 또한 웹앱을 수동으로 크기 조정할 수 있는 옵션이 있습니다.

1. 기본 옵션인 **메트릭 기준 스케일링**을 선택한 채로 두고 **+ 규칙 추가**를 클릭합니다
...
1. **추가**를 클릭하고 App Service 계획 스케일링 블레이드로 돌아가서 다음 설정을 지정합니다(다른 설정은 기본값으로 유지).
...
1. Cloud Shell 창에서 다음 명령을 실행하여 HTTP 요청을 웹앱으로 보내는 시작 루프와 무한 루프를 실행합니다.

...
1. Cloud Shell 창의 도구 모음에서 **파일 업로드/다운로드** 아이콘을 클릭합니다. 드롭다운 메뉴에서 **업로드**를 클릭하고 **\\Allfiles\\Labs\\06\\az104-06-vms-template.json**, **\\Allfiles\\Labs\\06\\az104-06-vm-template.json**, **\\Allfiles\\Labs\\06\\az104-06-vm-parameters.json** 파일을 Cloud Shell 홈 디렉터리에 업로드합니다.
...
1. 다음 설정을 사용하여 피어링을 추가합니다.(다른 설정은 기본값으로 유지.)
    ...
    | 설정 | 값 |
    | --- | --- |
    | 해당 가상 네트워크: 피어링 링크 이름 | **az104-06-vnet01_to_az104-06-vnet2** |
    | 원격 가상 네트워크로의 트래픽 | **허용(기본값)** |
    | 원격 가상 네트워크에서 전달된 트래픽 | **해당 가상 네트워크 외부에서 발생하는 트래픽 차단** |
...
1. 다음 설정을 사용하여 피어링을 추가합니다.(다른 설정은 기본값으로 유지.)
    ...
    | 설정 | 값 |
    | --- | --- |
    | 해당 가상 네트워크: 피어링 링크 이름 | **az104-06-vnet01_to_az104-06-vnet3** |
    | 원격 가상 네트워크로의 트래픽 | **허용(기본값)** |
    | 원격 가상 네트워크에서 전달된 트래픽 | **해당 가상 네트워크 외부에서 발생하는 트래픽 차단** |
    | 가상 네트워크 게이트웨이 | **없음(기본값)** |

...
>**참고**: 본 랩은 기본적으로 Standard_D2s_v3 SKU의 4개의 Azure VM 배포가 포함되기 때문에, 배포를 위해 선택한 지역의 Standard_Dsv3 시리즈에서 사용할 수 있는 총 8개의 vCPU가 필요합니다. 학생이 평가판 계정을 사용하며 vCPU가 4개로 제한되어 있는 경우 vCPU가 하나만 필요한 VM 크기(예: Standard_B1s)를 사용할 수 있습니다. 
...
이 작업에서는 동일한 Azure 지역에 4개의 가상 컴퓨터를 배포합니다. 처음 두 개는 허브 가상 네트워크에 상주하고 나머지 각각은 별도의 스포크 가상 네트워크에 상주합니다.
...
1. Cloud Shell 창에서 다음 명령을 실행하여 첫 번째 가상 네트워크와 가상 머신 쌍을 호스팅할 첫 번째 리소스 그룹을 만듭니다. 이때 `[Azure_region]` 자리 표시자는 Azure 가상 머신을 배포할 Azure 지역 이름으로 바꿉니다. "(Get-AzLocation).Location" cmdlet을 사용하면 지역 목록을 확인할 수 있습니다.
   ...
   ```
1. Cloud Shell 창에서 다음 명령을 실행하여 첫 번째 가상 네트워크를 만들고 업로드한 템플릿 및 매개 변수 파일을 사용하여 가상 컴퓨터 쌍을 배포합니다.
...
1. Cloud Shell 창에서 다음을 실행하여 두 번째 가상 네트워크 및 세 번째 가상 컴퓨터를 호스트하는 두 번째 리소스 그룹을 만듭니다.
...
1. Cloud Shell 창에서 다음을 실행하여 두 번째 가상 네트워크를 만들고 업로드한 템플릿 및 매개 변수 파일을 사용하여 가상 머신을 배포합니다.
   ...
   New-AzResourceGroupDeployment `
      ...
      -AsJob
   ```
1. Cloud Shell 창에서 다음 명령을 실행하여 세 번째 가상 네트워크 및 네 번째 가상 컴퓨터를 호스트하는 세 번째 리소스 그룹을 만듭니다.
...
1. Cloud Shell 창에서 세 번째 가상 네트워크를 만들고 업로드한 템플릿 및 매개 변수 파일을 사용하여 가상 머신을 배포합니다.
   ...
   New-AzResourceGroupDeployment `
      ...
      -AsJob
   ```
    >**참고**: 다음 작업으로 진행하기 전에 배포가 완료될 때까지 기다립니다. 약 5분이 소요됩니다.
...
1. 다음 설정을 사용하여 피어링을 추가합니다.(다른 설정은 기본값으로 유지.)
    ...
    | az104-06-vnet2에서 az104-06-vnet01으로 피어링 이름 | **az104-06-vnet2_to_az104-06-vnet01** |
    | az104-06-vnet01에서 az104-06-vnet2로의 가상 네트워크 액세스 허용 | **사용** |
    | az104-06-vnet2에서 az104-06-vnet01로의 가상 네트워크 액세스 허용 | **사용** |
    | az104-06-vnet2에서 az104-06-vnet01로의 전달된 트래픽 허용 | **사용 안 함** |
    | az104-06-vnet01에서 az104-06-vnet2로의 전달된 트래픽 허용 | **사용** |
...
1. 다음 설정을 사용하여 피어링을 추가합니다.(다른 설정은 기본값으로 유지.)
    ...
    | az104-06-vnet3에서 az104-06-vnet01으로 피어링 이름 | **az104-06-vnet3_to_az104-06-vnet01** |
    | az104-06-vnet01에서 az104-06-vnet3로의 가상 네트워크 액세스 허용 | **사용** |
    | az104-06-vnet3에서 az104-06-vnet01로의 가상 네트워크 액세스 허용 | **사용** |
    | az104-06-vnet3에서 az104-06-vnet01로의 전달된 트래픽 허용 | **사용 안 함** |
    | az104-06-vnet01에서 az104-06-vnet3로의 전달된 트래픽 허용 | **사용** |
    | 게이트웨이 전송 허용 | **(확인란 선택 취소)** |
...
1. **확인**을 클릭하고 연결 확인 결과가 반환될 때까지 기다립니다. **연결 가능** 상태인지 확인합니다. 네트워크 경로를 검토하고 VM 간 중간 홉이 없고 대기 시간이 최소화된 직접적 연결입니다.
    ...
    > **참고**: **az104-06-vm0**에서 Network Watcher 에이전트 가상 컴퓨터 확장을 설치해야 하기 때문에 초기 검사는 약 2분 정도 소요될 수 있습니다.
...
1. 다음 설정을 사용하여 새 경로를 추가합니다(다른 설정은 기본값으로 유지).

    | 설정 | 값 |
    | --- | --- |
    | 라우터 이름 | **az104-06-route-vnet2-to-vnet3** |
    | 주소 접두사 | **10.63.0.0/20** |
    | 다음 홉 유형 | **가상 어플라이언스** |
    | 다음 홉 주소 | **10.60.0.4** |
...
1. 다음 설정을 사용하여 새 경로를 추가합니다(다른 설정은 기본값으로 유지).
    ...
    | 설정 | 값 |
    | --- | --- |
    | 라우터 이름 | **az104-06-route-vnet3-to-vnet2** |
    | 주소 접두사 | **10.62.0.0/20** |
    | 다음 홉 유형 | **가상 어플라이언스** |
    | 다음 홉 주소 | **10.60.0.4** |
...
1. 다음 설정을 사용하여 부하 분산 장치를 만듭니다(다른 설정은 기본값으로 유지).
...
1. 다음 설정을 사용하여 상태 프로브를 추가합니다(다른 설정은 기본값으로 유지).
...
1. 상태 프로브가 만들어질 때까지 기다린 다음, **부하 분산 규칙**을 클릭한 다음 **+ 추가**를 클릭합니다.
...
1. 다음 설정을 사용하여 부하 분산 규칙을 추가합니다(다른 설정은 기본값으로 유지).
    ...
    | 아웃바운드 규칙을 사용하여 백 엔드 풀 멤버에게 인터넷 액세스 권한을 제공합니다. | **사용** |
...
1. Azure Portal에서 **Application Gateway**를 검색하여 선택하고 **Application Gateway** 블레이드에서 **+ 추가**를 클릭합니다.
...
1. **다음: 프런트 엔드 >**를 차례로 클릭하고 **애플리케이션 게이트웨이 만들기** 블레이드의 **프런트 엔드** 탭에서 다음 설정을 지정합니다(다른 설정은 기본값으로 유지).

    | 설정 | 값 |
    | --- | --- |
    | 프런트 엔드 IP 주소 유형 | **공용** |
    | 방화벽 공용 IP 주소| 새 공용 IP 주소 **az104-06-pip5**의 이름 |
...
1. **라우팅 규칙 추가** 블레이드의 **수신기** 탭에서 다음 설정을 지정합니다(다른 설정은 기본값으로 유지).
...
1. **라우팅 규칙 추가** 블레이드의 **백 엔드 대상** 탭에서 **HTTP 설정** 텍스트 상자 옆의 **새 항목 추가**를 클릭하고 **HTTP 설정 추가** 블레이드에서 다음 설정을 지정합니다(다른 설정 기본값으로 유지).
...
1. **다음: ****다음:** 뒤의 **태그 >** **검토 + 만들기>** 를 차례로 클릭한 후에 **만들기**를 클릭합니다.

    > **참고**: Application Gateway 인스턴스가 만들어질 때까지 기다리세요. 약 8분이 소요됩니다.

1. Azure Portal에서 **Application Gateway**를 선택하세요. **Application Gateway** 블레이드에서 **az104-06-appgw5** 를 클릭합니다.

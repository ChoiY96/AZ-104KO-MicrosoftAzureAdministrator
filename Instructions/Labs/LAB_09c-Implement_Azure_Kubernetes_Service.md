...
Contoso에는 Azure Container Instances를 사용하여 실행하기에는 적합하지 않은 많은 다중 계층 애플리케이션이 있습니다. 컨테이너화된 워크로드로 실행할 수 있는지 여부를 확인하기 위해 Kubernetes를 컨테이너 오케스트레이터로 사용하여 평가하려고 합니다. 관리 오버헤드를 더욱 최소화하기 위해 간소화된 배포 환경 및 크기 조정 기능을 포함하여 Azure Kubernetes Service를 테스트하려고 합니다.
...
1. **Kubernetes 클러스터 만들기** 블레이드의 **기본** 탭에서 다음 설정을 지정합니다(다른 설정은 기본값으로 남겨둡니다).
    ...
    | Kubernetes 버전 | 기본값을 적용합니다 |
    | 노드 크기 | 기본값을 적용합니다 |
    | 노드 수 | **1** |

1. **다음:**을 클릭합니다.** 노드 풀 >**, **Kubernetes 클러스터 만들기** 블레이드의 **노드 풀** 탭에서 다음 설정을 지정합니다(다른 설정은 기본값으로 유지).
...
1. **다음: 네트워킹 >**, **Kubernetes 클러스터 만들기** 블레이드의 **네트워킹** 탭에서 다음 설정을 지정합니다(다른 설정은 기본 값으로 유지).
...
1. **Cloud Shell** 창에서 다음 명령을 실행하여 Kubernetes Pod가 제대로 생성되었는지 확인합니다.
...
#### 작업 4: Azure Kubernetes Service 클러스터에서 컨테이너화된 워크로드 확장

이 작업에서는 Pod 갯수와 클러스터 노드 갯수를 가로로 조정합니다.

1. **Cloud Shell** 창에서 다음을 실행하여 Pod 수를 2로 늘려 배포를 확장합니다.
...
1. **Cloud Shell** 창에서 노드 수를 2로 늘려 클러스터를 확장하려면 다음을 실행합니다. 

    ```sh
    az aks scale --resource-group $RESOURCE_GROUP --name $AKS_CLUSTER --node-count 2
    ```

    > **참고**: 추가 노드의 프로비전이 완료될 때까지 기다립니다. 배포가 완료되려면 3분 정도 걸립니다. 실패한 경우 `az aks scale` 명령을 다시 실행합니다.

1. **Cloud Shell** 창에서 다음 명령을 실행하여 클러스터 크기가 잘 조정되었는지 확인합니다.
...
1. **Cloud Shell** 창에서 다음을 실행하여 배포를 스케일링합니다.
...
- Pod를 Azure Kubernetes Service 클러스터에 배포했습니다
- Azure Kubernetes Service 클러스터에서 컨테이너화된 워크로드를 스케일링했습니다
